...
public class DivByZeroVisitor extends BaseTypeVisitor<DivByZeroAnnotatedTypeFactory> {
    ...
    private boolean errorAt(BinaryTree node) {
        ...
        if (DIVISION_OPERATORS.contains(node.getKind())) {
            ...
            || hasAnnotation(node.getRightOperand(), NegZero.class)
            || hasAnnotation(node.getRightOperand(), PosZero.class)
    ...
    private boolean errorAt(CompoundAssignmentTree node) {
        ...
        // such as "x += 10".
        if (DIVISION_OPERATORS.contains(node.getKind())) {
            if(hasAnnotation(node.getVariable(), Zero.class)
            || hasAnnotation(node.getVariable(), NegZero.class)
            || hasAnnotation(node.getVariable(), PosZero.class)
